...
async def 도움(ctx):
    embed = discord.Embed(title="씨부엉이", description="밤샘 코딩 하는 씨부엉이입니다.\n 소스코드 : https://github.com/sive2045/discord_bot", color=0x4432a8) 
    embed.add_field(name="1. 인사", value='!hello, !안녕, !hi\n 부엉이에게 인사해보세요!')
    embed.add_field(name="2. 제비뽑기 참여", value='!참여, !참가\n 제비 뽑기에 참여합니다!\n !참여자를 통해 참여 여부를 확인할 수 있습니다!')    
    
    await ctx.send(embed=embed)

@bot.command(aliases=['열기'])
async def 제비뽑기_열기(ctx):
    if len(ctx.author.roles) > 1:
        global check
        check = True
        await ctx.send("제비뽑기를 열었습니당!")

@bot.command(aliases=['닫기'])
async def 제비뽑기_닫기(ctx):
    if len(ctx.author.roles) > 1:
        global check
        check = False
        await ctx.send("제비뽑기를 닫았습니당!")

@bot.command(aliases=['참가'])
async def 참여(ctx):
    global user
    global check
    if check:
        ...
        await ctx.send("제비뽑기 참가 가능 시간이 아닙니다!")
